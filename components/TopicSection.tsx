  ...
  return (
    <section
      ...
      <div className="flex flex-wrap items-center gap-x-7 gap-y-2">
        ...
        {block.links.map((link) => (
          <Link
            key={link.label}
            href={link.href}
            className="text-[15px] text-neutral-700 hover:text-flagGreen">
            {link.label}
          </Link>
        ))}
      </div>
      ...
      <div className="mt-6 grid gap-8 lg:grid-cols-[1fr_1.55fr]">
        <div>
          <h3 className="text-[30px] font-black leading-[1.08] tracking-tight text-ink">
            <Link href={`/tours/${block.lead.slug}`} className="hover:text-flagGreen">
              {block.lead.title}
            </Link>
          </h3>
          ...
          <div className="mt-6 border-t border-rule pt-4">
            ...
            <FeaturePanel>
              <StaggerReveal className="grid grid-cols-3 gap-3 p-3">
